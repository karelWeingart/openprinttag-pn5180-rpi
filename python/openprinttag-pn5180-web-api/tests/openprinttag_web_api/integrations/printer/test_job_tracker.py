...
def mock_sleep(mocker: pytest_mock.plugin.MockerFixture) -> AsyncMock:
    return mocker.patch("openprinttag_web_api.integrations.printer.job_tracker.asyncio.sleep")
...
async def test_get_gcode_metadata_for_jobs() -> None:
    ...
    job_tracker_jobs.update({1: JobWatcherState(
        job_id=1,
    ...
    try:
        _completed: int = await get_gcode_metadata_for_jobs(_client, max_gcode_download_attempts=3)
        assert _completed == 0
    finally:
        job_tracker_jobs.clear()
...
@pytest.mark.anyio
@pytest.mark.parametrize("max_attempts, number_of_failures", [
    (4, 4),
    (5, 3),
    (2, 3)
])
async def test_get_gcode_metadata_after_finished_printing(max_attempts: int, 
                                                        number_of_failures: int) -> None:
    ...
    _client.get_gcode_metadata.side_effect = (
        [httpx.HTTPError("fail")] * number_of_failures + [_SUCCESS_METADATA]
    )
    try:
        for _attempt in range(number_of_failures):
            _completed = await get_gcode_metadata_for_jobs(_client, max_gcode_download_attempts=max_attempts)
...
async def test_get_gcode_metadata_during_printing() -> None:
    ...
    _client = AsyncMock()
    _max_attempts = 3
    _side_effects = [httpx.HTTPError("fail")] * (_max_attempts + 1) + [_SUCCESS_METADATA]
    ...
    try:
        ...
        for _ in range(len(_side_effects)):
            assert _completed == 0
            _completed = await get_gcode_metadata_for_jobs(_client, max_gcode_download_attempts=_max_attempts)
...
@pytest.mark.anyio
async def test_watch_jobs_not_downloaded(mocker: pytest_mock.plugin.MockerFixture, mock_sleep: AsyncMock, mock_client: AsyncMock) -> None:
    ...
    _mock_get_gcode_metadata = mocker.patch("openprinttag_web_api.integrations.printer.job_tracker.get_gcode_metadata_for_jobs")
...
@pytest.mark.anyio
async def test_watch_jobs_fully_completed(mock_sleep: AsyncMock, mock_client: AsyncMock) -> None:
    """Test that watch_jobs yields a CompletedJobDto when a job finishes."""
    job_tracker_jobs.clear()
...
@pytest.mark.anyio
async def test_watch_jobs_never_downloaded_job(mock_sleep: AsyncMock, mock_client: AsyncMock) -> None:
    """Test that watch_jobs yields a CompletedJobDto when a job finishes."""
    job_tracker_jobs.clear()
...
@pytest.mark.anyio
async def test_watch_jobs_another_job(mock_sleep: AsyncMock, mock_client: AsyncMock) -> None:
    """Test that watch_jobs yields a CompletedJobDto when a job finishes."""
    job_tracker_jobs.clear()

    mock_client.get_job.side_effect = [_job_response, _second_job_response, None, None]
    mock_client.get_gcode_metadata.side_effect = [None, None, None, None, None, None, None]
    mock_sleep.side_effect = [None, None, None, asyncio.CancelledError("test")]
