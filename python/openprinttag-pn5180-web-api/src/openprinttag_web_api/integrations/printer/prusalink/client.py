...
class PrusaLinkClient:
    ...
    async def get_file_metadata(
        self, file_path: str, header_size: int = 4096
    ) -> GcodeMetadata:
...
async def watch_job(
    ...
    if job.id != state.job_id:
        ...
        _metadata: GcodeMetadata = await client.get_file_metadata(
            download_ref, header_size=16384
        )
        print(
            f"Parsed metadata: filament_type={_metadata.filament_type}, filament_used_g={_metadata.filament_used_g}"
        )
    ...
    if state.metadata and state.metadata.filament_used_g is not None:
        ...
        print(
            f"Progress: {job.progress}% | Used: {usage['used_g']:.2f}g / {usage['total_g']:.2f}g"
        )
    elif state.metadata:
        print(
            f"Progress: {job.progress}% | Filament data not available (filament_used_g={state.metadata.filament_used_g})"
        )
    ...
    while True:
        try:
            ...
            if job:
                ...
                if state.job_id:
                    end_reason = (
                        "completed" if state.last_progress >= 99.0 else "canceled"
                    )
