...
async def get_printers(
    page: int,
    page_size: int,
    printer_repository: Annotated[
        SqlitePrinterRepository, Depends(SqlitePrinterRepository)
    ],
) -> PrinterListResponse:
    _offset: int = (page - 1) * page_size
    _total: int = printer_repository.count()
    _printers: list[PrinterRecord] = printer_repository.list_all(
        offset=_offset, page_size=page_size
    )
    return PrinterListResponse(
        printers=[Printer.from_orm(_p) for _p in _printers],
        total=_total,
...
async def add_printer(
    ...
    if not _printer:
        raise HTTPException(
            status_code=404,
            detail=f"Printer with id {_printer_id} not found after creation!",
        )
...
async def delete_printer(
    ...
    if not _printer:
        raise HTTPException(
            status_code=404, detail=f"Printer with id {printer_id} not found!"
        )
...
async def set_active_printer(
    printer_id: Annotated[int, Path(description="ID of the printer to set as active")],
    printer_repository: Annotated[
        SqlitePrinterRepository, Depends(SqlitePrinterRepository)
    ],
    printer_connector: Annotated[
        PrinterConnectorService, Depends(get_printer_connector)
    ],
) -> Printer:
    _printer: PrinterRecord | None = printer_repository.get_by_id(printer_id)
    _current_active_printer: PrinterRecord | None = printer_repository.get_by_status(
        "ENABLED"
    )
    if not _printer:
        raise HTTPException(
