...
class SqlitePrinterRepository(PrinterRepository):
  ...
  def get_by_status(self, status: str) -> PrinterRecord | None:
    with get_db() as db:
      ...
      if row is None:
        return None
      return PrinterRecord(id=row["id"],
                            name=row["name"],
                            status=row["status"], 
                            ip=row["ip"], 
                            token=row["token"])
  ...
  def delete(self, printer_id: int) -> None:
    with get_db() as db:
      db.execute(f"DELETE FROM {self._sql_table} WHERE id = ?", (printer_id,),)
      db.commit()
  ...
  def list_all(self,
    ...
    with get_db() as db:
      ...
      _printers = [PrinterRecord(id=_row["id"],
                                name=_row["name"],
                                status=_row["status"],
                                ip=_row["ip"], 
                                token=_row["token"]) for _row in _rows]
      return _printers
  
  def update(self, printer: PrinterRecord) -> PrinterRecord:
    with get_db() as db:
      _cursor = db.execute(f"""UPDATE {self._sql_table} 
                        SET 
                          status = ?,
                          ip = ?,
                          name = ?,
                          token = ?
                        WHERE id = ?""",(printer.status, printer.ip,printer.name, printer.token, printer.id,))
