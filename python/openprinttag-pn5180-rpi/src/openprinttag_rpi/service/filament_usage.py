...
def save_filament_usage_message(
    job_id: str, tag_uid: str, filament_usage: float, status: str = "NOT_PROCESSED"
) -> int:
...
def cancel_not_processed_messages_for_tag(tag_uid: str) -> int:
    ...
    _jobs: list[FilamentUsageMessage] = (
        _filament_usage_message_repository.find_by_status_and_by_tag_uid(
            tag_uid, "NOT_PROCESSED"
        )
    )
    for job in _jobs:
        job.status = "CANCELED"
...
def get_total_filament_usage_by_tag_uid_status(
    tag_uid: str, status: str = "NOT_PROCESSED"
) -> float:
    """Gets the total filament usage for a given tag UID and status."""
    return _filament_usage_message_repository.get_usage_by_tag_uid_status(
        tag_uid, status
    )
