...
def make_mock_has_openprinttag_bin(mocker: pytest_mock.plugin.MockerFixture) -> Mock:
    def _factory(side_effect: list[bool]) -> Mock:
        return mocker.patch("openprinttag_rpi.openprinttag.reader.has_openprinttag_bin", side_effect=side_effect)
    return _factory
...
def test_search_tag_has_openprinttag_bin(mocker: pytest_mock.plugin.MockerFixture, 
                    mock_reader: Mock, 
                    make_mock_has_openprinttag_bin: Mock) -> None:
    """Test the search_tag function."""
    make_mock_has_openprinttag_bin([True])
    _uid = search_tag(mock_reader)
    assert _uid is None

def test_search_tag_invalid_uid_then_has_openprinttag_bin(mocker: pytest_mock.plugin.MockerFixture,
                    mock_reader: Mock, 
                    make_mock_has_openprinttag_bin: Mock,
                    make_spy_from_reader_module: Mock) -> None:
    """Test the search_tag function."""
    ...
    _uid = search_tag(mock_reader)
    _expected_event_dto = EventDto(event_type=TagReadEvent.TAG_UID_INVALID, data={"uid": _invalid_uid})
    spy_register_event.assert_called_once_with(_expected_event_dto)
    assert _uid is None

def test_search_tag_valid_uid(mocker: pytest_mock.plugin.MockerFixture,
                        mock_reader: Mock, 
                    make_mock_has_openprinttag_bin: Mock,
                    make_spy_from_reader_module: Mock) -> None:
    """Test the search_tag function."""
    ...
    _uid = search_tag(mock_reader)
    _expected_event_dto = EventDto(event_type=TagReadEvent.TAG_DETECTED, data={"uid": _uid})
    spy_register_event.assert_called_once_with(_expected_event_dto)
