...
class ExtendedISO15693Sensor(ISO15693Sensor):
    ...
    def __command(self, frame: bytearray) -> bytearray:
        self.__pre_command()
        ...
        try:            
            ...
            self._log(f"Exception in get_system_info: {e}")
            raise GetSystemInfoError("Error getting system-info") from e
        finally:
            self.__post_command()
    ...
    def parse_system_info(self, system_info_bytes: bytes) -> Optional[Dict[str, Any]]:
        ...
        if not system_info_bytes or len(system_info_bytes) < 10:
            self._log(f"System info too short: {len(system_info_bytes) if system_info_bytes else 0} bytes")
            return None
    ...
    def read_multi_blocks(self, blocks: int) -> bytes:
        ...
        for _block in range(blocks):
            ...
            while _data is None:
                _data = self.read_single_block(_block)

                if time.time() - _block_start > self._BLOCK_READ_TIMEOUT:
                    print(f"\nTimeout reading block {_block}, aborting.")
                    break

            if _data is None:
                break
            print(f"Reading block {_block}", end="\r")
            _raw += _data
        return _raw